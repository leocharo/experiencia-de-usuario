...
if (deleteAccountBtn) {
    deleteAccountBtn.addEventListener('click', async () => {
        ...
        if (primeraConfirmacion) {
            // Segunda confirmación por seguridad (escribir palabra clave)
            ...
            if (confirmacionPalabra === "ELIMINAR") {
                ...
                if (user) {
                    try {
                        // A. Borrar el documento del usuario en Firestore (la colección "perfiles")
                        const profileRef = doc(db, "perfiles", user.uid);
                        await deleteDoc(profileRef);
                        console.log("Documento de Firestore eliminado con éxito.");

                        // B. Borrar al usuario de Firebase Authentication
                        ...
                        // Manejo de error específico de Firebase: Sesión antigua
